...
import { GenerateMultiRequestSchema, GenerateResponseSchema, GenerateResponse } from '../schemas'
import { SYSTEM_PROMPT, buildMultiPrompt } from '../prompt'

export const generateMultiRouter = express.Router()

generateMultiRouter.post('/', async (req: express.Request, res: express.Response): Promise<void> => {
  try {
    // Validate request body
    const validationResult = GenerateMultiRequestSchema.safeParse(req.body)
    ...
    // Build prompts for multiple stories
    const userPrompt = buildMultiPrompt(request.stories, request.additionalInfo)
    ...
    console.error('Error in generate multi route:', error)
    res.status(500).json({
